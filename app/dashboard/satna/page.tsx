...
export default function NewsPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editId) {
        await axios.put(
          `https://starnewsbackend.onrender.com/api/satna/${editId}`,
          formData
        );
      } else {
        await axios.post(
  ...
  const handleDelete = async (id: string) => {
    ...
    setLoadingId(id);
    await axios.delete(`https://starnewsbackend.onrender.com/api/satna/${id}`);
    fetchNews();
    setLoadingId(null);
  };
  ...
  return (
    <div className="p-4 sm:p-6 bg-black text-white min-h-screen">
      <div className="flex flex-col sm:flex-row justify-between gap-3 mb-6">
        <h1 className="text-xl sm:text-3xl font-bold">📰Satna News</h1>
        <button
